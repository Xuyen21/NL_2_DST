...
from utils.promptfoo_result_utils import extract_prompt_variant, load_provider_models

PROJECT_ROOT = Path(__file__).resolve().parent
RESULTS_PATH = PROJECT_ROOT / "evaluations" / "results-zero-shot-cot-grok.json"
PROMPTFOO_CONFIG_PATH = PROJECT_ROOT / "evaluations" / "promptfoo_eval" / "promptfooconfig_pilot.yaml"
...
def build_output_path(output_dir: Path, story_title: str, model_name: str, provider_variant: str) -> Path:
	base_name = f"results-{slugify(story_title)}-{slugify(model_name)}-{slugify(provider_variant)}"
...
def export_promptfoo_results(
	...
	for entry in report.get("results", {}).get("results", []):
		...
		output_path = build_output_path(output_dir, story_title, model_name, prompt_variant)
